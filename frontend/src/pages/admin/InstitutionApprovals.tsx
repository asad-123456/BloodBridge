...
export function InstitutionApprovals() {
  ...
  const handleApprove = async (id: string, approved: boolean, reason: string) => {
    try {
      await approveInstitution(id, approved, reason);
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search size={17} className="absolute left-3 top-3 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search institutions" className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm outline-none focus:border-primary" /></div><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold"><option>Pending</option><option>All</option><option>Approved</option><option>Rejected</option></select></div>
      ...
      {visible.length === 0 && (
        <div className="flex flex-col items-center justify-center px-5 py-12 text-center">
          <div className="grid size-12 place-items-center rounded-full bg-slate-50 text-slate-500">
            <SearchX size={24} />
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">{current ? <><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Review detail</p><h2 className="mt-2 font-bold text-slate-900">{current.name}</h2><dl className="mt-5 space-y-3 text-sm"><div className="flex justify-between gap-3"><dt className="text-slate-500">Type</dt><dd className="font-semibold text-right">{current.facilityType || current.type}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">License #</dt><dd className="font-semibold text-right">{current.licenseNumber || "N/A"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Contact Person</dt><dd className="font-semibold text-right">{current.contactPersonName || current.contactName}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Designation</dt><dd className="font-semibold text-right">{current.contactPersonDesignation || "N/A"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Phone</dt><dd className="font-semibold text-right">{current.phone}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Website</dt><dd className="font-semibold text-right">{current.websiteUrl ? <a href={current.websiteUrl} target="_blank" rel="noreferrer" className="text-primary hover:underline">{current.websiteUrl}</a> : "N/A"}</dd></div></dl>{current.registrationStatus === "Pending" && <div className="mt-6 grid gap-2"><button onClick={() => handleApprove(current.id, true, "Approved after admin review.")} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2.5 text-sm font-bold text-white"><Check size={16} /> Approve institution</button><button onClick={() => handleApprove(current.id, false, "Registration requires further review.")} className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-600"><X size={16} /> Reject registration</button></div>}</> : <p className="py-8 text-center text-sm text-slate-500">Select an institution to review.</p>}</div>
      </div>
    </>
  );
}
